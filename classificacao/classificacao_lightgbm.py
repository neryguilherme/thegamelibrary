...
lgb_model = lgb.LGBMClassifier(objective='multiclass', num_class=len(np.unique(y_train)), random_state=42)
...
precision_lgb_train = precision_score(y_train, y_pred_lgb_train, average='weighted', zero_division=0.0)
recall_lgb_train = recall_score(y_train, y_pred_lgb_train, average='weighted', zero_division=0.0)
f1_lgb_train = f1_score(y_train, y_pred_lgb_train, average='weighted', zero_division=0.0)

# Métricas para o conjunto de teste
accuracy_lgb_test = accuracy_score(y_test, y_pred_lgb_test)
precision_lgb_test = precision_score(y_test, y_pred_lgb_test, average='weighted', zero_division=0.0)
recall_lgb_test = recall_score(y_test, y_pred_lgb_test, average='weighted', zero_division=0.0)
f1_lgb_test = f1_score(y_test, y_pred_lgb_test, average='weighted', zero_division=0.0)

# Imprimir as métricas
print(classification_report(y_test, y_pred_lgb_test, zero_division=0.0))
print("-" * 50)
print(f"Acurácia no conjunto de treinamento (LightGBM): {accuracy_lgb_train}")
print(f"Precisão no conjunto de treinamento (LightGBM): {precision_lgb_train}")
print(f"Recall no conjunto de treinamento (LightGBM): {recall_lgb_train}")
print(f"F1-score no conjunto de treinamento (LightGBM): {f1_lgb_train}")
print("-" * 50)
print(f"Acurácia no conjunto de teste (LightGBM): {accuracy_lgb_test}")
print(f"Precisão no conjunto de teste (LightGBM): {precision_lgb_test}")
print(f"Recall no conjunto de teste (LightGBM): {recall_lgb_test}")
print(f"F1-score no conjunto de teste (LightGBM): {f1_lgb_test}")